...
**Depends on:** Task 3 (consumes its shared `refuse` helper; the helper's exact import path and signature come from the Task 3 completion report — the shared helper is `crate::refusal::refuse(stream: &mut TcpStream, response: &[u8])` in `src/refusal.rs`, with `REFUSAL_DRAIN_READS = 32` beside it). **Parallel-safe with:** Tasks 4, 5, 6, 7, 8 (no shared files).
...
- **Once a scope-change event has begun, no `send` and no debugger event for an affected tab passes until membership has been recomputed.** Scope checks that begin with an `await` cannot provide this on their own — every scope-affecting listener must take a synchronous suppression step before its first `await`.
- **Release is fail-closed.** A tombstone is only ever released into a *recomputed known-good* state. If a recompute cannot determine membership — it throws anywhere in its body — every tab it captured is revoked (deleted from `announced`, best-effort force-detached, `tabRemoved` posted) *before* the release. "Could not determine membership" resolves to "not in scope", never to "unchanged".
...
6. Membership helpers — **exact title equality, never the query's pattern semantics** (`chrome.tabGroups.query({ title })` matches titles against a *pattern*, so filter the result):
   - `async function ompGroupIds(): Promise<Set<number>>` — `(await chrome.tabGroups.query({ title: OMP_GROUP.title })).filter(g => g.title === OMP_GROUP.title)` → set of ids (all windows); errors → empty set. (Catching to empty fail-closes only the *group lookup*; the fail-closed release in step 10 exists because a recompute can also throw in `chrome.tabs.get`/`chrome.tabs.query` or while applying the transition.)
...
10. **Evict immediately, tombstone synchronously, release fail-closed** — every scope-affecting listener suspends *before its first `await`*, then queues the recompute through `enqueueScopeOp`. Central helper `queueScopeCheck(tabId: number): void` — caller has already called `suspend(tabId)`; the enqueued body:
```ts
enqueueScopeOp(async () => {
	try {
		const tab = await chrome.tabs.get(tabId).catch(() => null); // missing tab → out of scope
		const member = tab !== null && !allTabs ? (await ompGroupIds()).has(tab.groupId) : tab !== null;
		// transition matrix over (member, announced.has(tabId)) — see below
	} catch {
		// fail closed: membership could not be determined → revoke, never restore
...
```
    - Transition matrix: member + announced → post `tabUpdated`; member + new → add to `announced`, post `tabCreated` (a tab dragged *into* the group becomes drivable); non-member + announced → delete from `announced`, `await chrome.debugger.detach({ tabId }).catch(() => {})`, post `tabRemoved` — dragging a tab out revokes access mid-session; non-member + unknown → nothing (ungrouped tabs are never announced).
    ...
    - `chrome.tabGroups.onCreated/onUpdated/onRemoved`: **pessimistic synchronous prologue** — `if (allTabs) return; const ids = [...announced]; for (const id of ids) suspend(id);` then enqueue one `reconcileScope()` op with the same fail-closed shape: `try { chrome.tabs.query({}) + fresh ompGroupIds() + transition matrix per tab } catch { for (const id of ids) { await chrome.debugger.detach({ tabId: id }).catch(() => {}); if (announced.delete(id)) post({ t: "tabRemoved", tabId: id }); } } finally { for (const id of ids) release(id); }` — a reconciliation that fails revokes every tab it captured rather than restoring the prior announced set. This covers a group being retitled to/from `"omp"` or dissolved, which fires no per-tab events. (A member wrongly revoked by a failed reconcile self-heals: any later tab event or the next hello re-announces it — revocation is the safe direction.)
...
- **The fail-closed release test** (locks the revoke-on-failure invariant): failure mode = a failed reconciliation restores a stale `announced` set and events leak after release.
  1. Arrange announced tab 1 in omp group 5 as above (hello done, config delivered).
  2. `fake.failNextTabsQuery(new Error("boom"));` emit `tabGroups.onUpdated({ id: 5, windowId: 1, title: "omp" })` — the pessimistic prologue captures tab 1; the enqueued `reconcileScope` body throws at `chrome.tabs.query`.
  3. Flush → assert the fail-closed branch ran: `debugger.detach` was called for tab 1 and the socket posted `{"t":"tabRemoved","tabId":1}` — the tab ends up **detached and retracted, not announced**.
  4. After the tombstone is released, emit `debugger.onEvent({ tabId: 1 }, "Page.loadEventFired", {})` → assert **no `cdpEvent` is posted** (the filter now blocks on `!announced`, proving the release did not restore the stale prior state).
...
**Verify (devbox):** `cd packages/browser-relay && bun run check && bun test && bun run build` — check exits 0 (tsgo covers `background.ts`, `chrome.d.ts`, and the new tests against the new protocol), the race test, fail-closed test, and companions pass, build prints the three `built:` lines; `jj st` in the workspace shows `extension-assets/background.js.txt` modified. **Real Chrome (laptop) still validates the genuine event streams and drag gestures** — the checklist lands in Task 17 — but the revocation invariants themselves are now devbox-proven through the chrome fake.
...
2. `packages/browser-relay/CHANGELOG.md`, under `## [Unreleased]` (create the section if the file lacks one):
   - `### Changed`: the extension now enforces the `omp` tab group as the access-control list — it announces only grouped tabs, re-derives membership on reconnect, checks scope on every attach/send/remove/activate at execution time, suppresses commands and debugger events while a membership change is being recomputed (a reconciliation that fails revokes the affected tabs rather than restoring them), force-detaches and retracts tabs that leave the group (including cross-window drags), and closes a created tab it fails to group; the group is no longer dissolved when the relay disconnects.
...
**Where work happens:** `/home/ubuntu/secretsd/default` (jj-colocated clone of `github:sjawhar/secretsd`, `main` tracked — NOT the `/tmp/secretsd-plan-read` grounding clone). Version control is jj. Before editing, read `AGENTS.md` and `src/client/AGENTS.md`; the non-negotiables bind every line below (no plaintext at rest, fail closed, never render a secret or any prefix of it into an output or error).

**Depends:** none. **Parallel-safe with:** 24, 25, 27.

**Files:** `src/client/cli.rs`, `src/client/edit.rs`, `src/client/edit/new.rs`, `src/client/error.rs`, `src/hardening.rs`, `Cargo.toml` (only if the nix `fs` feature is missing); doc rows in `AGENTS.md`, `src/client/AGENTS.md`, `docs/design.md`.
...
2. `src/client/edit.rs` — add:
   ...
   ```
   Decisions locked in: `edit_arguments(_, 2, false)` accepts only `--source` (`--local` is rejected — a new key is always the `.local.env` variant, matching the per-key layout `DEEL_SESSION_COOKIE.local.env` / `ZIP_SESSION_COOKIE.local.env` already use, via `name.local_file_name()` inside `new_human_path`). An existing key rotates **its actual file** wherever it lives; reusing `existing_human_path` keeps the `--source`-vs-actual-root `EditConflict` check. Ambiguous keys are already refused upstream by `HumanNames::load` (`DuplicateHumanKey`). The created-vs-rotated report is computed under the write lock in `new::set_human` (below), not passed in from this racy pre-check.
3. `src/hardening.rs` — the existing hardening is daemon-side and is NOT on this write path (`src/bin/secrets.rs` never applies it before client commands). Extract the dumpability + core-limit steps that `apply(policy)` already performs (its doc note: "Dumpability and core limits are set before memlock") into a new `pub fn apply_no_core_dumps() -> Result<(), HardeningError>` — `RLIMIT_CORE = 0` via `nix::sys::resource::setrlimit` and `PR_SET_DUMPABLE = 0` via prctl, exactly the code `apply` uses today, with `apply` now delegating to it. Reuse, not a parallel `libc::setrlimit` call in the client: the repo already routes these syscalls through nix (`Cargo.toml`: "Syscalls: SO_PEERCRED, mlockall, prctl, openat"), and a second hand-rolled path would be a second convention. No mlockall for the client: it is short-lived, and the daemon-sized `MIN_MEMLOCK_BYTES` floor would fail-closed the client for no benefit. Update the module doc comment ("applied before the daemon holds any plaintext") to name the client write path as a second caller.
4. `src/client/edit/new.rs` — the non-interactive path writes **no plaintext temp file at all** (stronger than the editor flow; the runtime dir is never touched):
   - `pub(super) fn set_human(path: &Path, name: &SecretName) -> Result<(), CliError>`, in this order:
     1. `crate::hardening::apply_no_core_dumps().map_err(CliError::Hardening)?` — **before any stdin byte is read**, failing closed with no fallback: this is mandatory process-local hardening, never reliant on inherited shell or systemd limits, because this utility's no-leak guarantee is absolute on every machine. `RLIMIT_CORE=0` is inherited across fork/exec, so the spawned `sops` child cannot dump the plaintext either; `PR_SET_DUMPABLE=0` additionally protects this process (execve resets dumpability in the child, which is why the child's guard is the inherited core limit, not dumpability).
     2. `read_piped_assignment(name)?` (below).
     3. Take the per-directory write lock, then compute the rotation report race-free: open `path.parent()` (`O_RDONLY`; it is a directory) and take a **blocking exclusive flock** via `nix::fcntl::Flock::lock(file, FlockArg::LockExclusive)` (nix 0.29 is already a dependency; add its `fs` feature only if absent). Then `let rotated = path.exists();`. The lock is held through persist and released on drop. Locking the directory fd instead of a `.KEY.lock` sidecar is deliberate: `HumanNames` discovery validates every filename in `secrets.human.d/`, so a leftover sidecar would break `secrets list` for every caller; the directory fd leaves zero residue. Coarse per-directory serialization is fine — writes are rare and short. Document the contract in the doc comment: concurrent `set-human` invocations serialize on the target directory; the last writer wins, and each completed write is atomic.
     4. `encrypt_bytes(&assignment, path)?`, then print exactly one line to stdout — `created {path}` or `rotated {path}` (`path.display()`). No VCS interaction: the command commits nothing by design.
   - `fn read_piped_assignment(name: &SecretName) -> Result<SecretBytes, CliError>`: refuse a terminal stdin (`use std::io::IsTerminal;` → `CliError::SetHumanTerminalStdin`) so a bare interactive run can never hang or capture typed scrollback; `read_to_end` into a `Vec<u8>` (on failure zeroize the partial buffer, return `CliError::SetHumanRead(e)`); strip exactly one trailing `\n` (and a preceding `\r` if present); **then reject outright, zeroizing the buffer, if any `\n` or `\r` byte remains anywhere in the value** → `CliError::InvalidPipedHumanSecret(name)` — this raw-byte check is the actual multiline guard, because `parse_single_assignment` (`src/secret.rs`) skips blank and `#`-comment lines while parsing dotenv, so `a\n#comment` and `a\n\n` would otherwise slip through as one assignment; empty after strip → `CliError::EmptyPipedHumanSecret(name)`; build `KEY=<value>\n`, wrap in `SecretBytes` immediately and zeroize intermediates (mirror `PlaintextTemp::read`); only afterwards run `parse_single_assignment(bytes, name)`, purely as the dotenv-shape and anti-name-swap check.
   - Refactor: extract the sops argv from `encrypt()` into `fn sops_encrypt_command(directory: &Path, target: &Path) -> Command` (`sops encrypt --filename-override <target> --input-type dotenv --output-type dotenv`, `.current_dir(directory)`); `encrypt()` keeps `Stdio::from(file)` + `persist_noclobber` unchanged. New `fn encrypt_bytes(plaintext: &SecretBytes, target: &Path) -> Result<(), CliError>`: `Builder::new().prefix(".secretsd-ciphertext-").tempfile_in(parent)`, spawn with `Stdio::piped()` stdin, `write_all` the bytes, drop the stdin handle (sops consumes stdin fully before writing output — child stdout is a `File`, so a sequential write cannot deadlock), drain stderr to sink (never surface sops stderr: it ran with the plaintext on stdin), `wait()`, then `ciphertext.persist(target)` — **clobber, not `persist_noclobber`**: rotation is the point. Tempfile-in-target-dir + rename is atomic (never a torn ciphertext, addressing the scout's atomic-write flag).
   - Daemon cache invalidation needs **no daemon change** — record this in the doc comment: rotation replaces the inode, and the daemon snapshots `FileIdentity { device, inode, size, modified, changed }` (`src/store.rs:45-50`) via `HumanStore::identity` (`src/store.rs:166`), which `resolve_access` (`src/server/approval.rs:51-70`) compares to revoke stale grants.
5. `src/client/error.rs` — five new variants; the exhaustive `Display`/`source()` matches force arms for each: `SetHumanTerminalStdin` ("set-human reads the secret value from stdin; pipe it in — the value must never appear in argv"), `SetHumanRead(std::io::Error)`, `EmptyPipedHumanSecret(SecretName)`, `InvalidPipedHumanSecret(SecretName)` ("piped secret '{}' must be one single-line assignment value"), `Hardening(crate::hardening::HardeningError)` ("could not disable core dumps: {error}"; `source()` → `Some`). Never render the value. Extend the `Usage` string to insert `secrets set-human KEY [--source NAME]` after the `edit-human` clause — the literal substring `set-human` in `secrets --help` output is the availability-detection contract Task 24 greps.
6. Docs (same commit): root `AGENTS.md` "Where to look" row "Change human-secret creation" gains the set-human symbols; `src/client/AGENTS.md` file-table row for `edit/new.rs`; `docs/design.md` creation-flow section gains a set-human paragraph (stdin never argv, public-age-recipient encrypt so storing needs no YubiKey touch, no plaintext at rest, core dumps disabled on the write path, reading stays touch-gated).
...
# Hardening is applied before stdin is read — observe the live process on scratch state
# (never the real store; config format matches tests/client/fixture.rs):
d=$(mktemp -d) && mkdir -p "$d/root/secrets.human.d" && printf '[source.scratch]\npath = "%s/root"\n' "$d" > "$d/config.toml"
sleep 3 | env SECRETSD_CONFIG="$d/config.toml" ./target/debug/secrets set-human PROBE_KEY & pid=$!
sleep 1; grep 'Max core file size' "/proc/$pid/limits"    # soft 0, hard 0
stat -c %U "/proc/$pid"                                    # root — PR_SET_DUMPABLE=0 flips /proc/<pid> ownership
wait $pid; echo "exit=$?"                                  # nonzero is fine (empty stdin / no .sops.yaml in scratch); the probe is the two lines above
```
...
3. `tests/client/set_human.rs` — contract tests against the fake sops fixtures (`tests/fixtures/fake-sops-ok`, `fake-sops-fail`), reusing `Fixture::agent`/`Fixture::human`, `write_human_name_in`, `add_root`, `sops_log`/`sops_arguments`/`sops_calls`, `runtime_dir`, and edit.rs's assertion-helper patterns (`assert_sops_encrypt_command`, `assert_runtime_is_empty`, `assert_ciphertext_contains`, `FAKE_SOPS_CIPHERTEXT_MARKER`):
   ...
   - `set_human_refuses_multiline_and_comment_smuggling_values` — each of stdin `b"a\nb"`, `b"a\n#comment"`, `b"a\n\n"`, `b"a\r\nb"` is refused (the last three are the `parse_single_assignment` blank/comment-line gaps the raw `\n`/`\r` rejection exists to close); for each: nonzero exit, no file created, `sops_calls() == 0`, and the value bytes absent from stdout and stderr.
   - `set_human_strips_exactly_one_trailing_newline` — stdin `b"value\n"`; `assert_ciphertext_contains` the assignment `KEY=value\n`.
   - `set_human_zeroes_the_core_limit_before_reading_stdin` — spawn via `command()` with stdin piped and **held open**; poll `/proc/<child>/limits` (bounded retries) until the "Max core file size" row reads `0 0` while the child is still blocked on stdin, then close stdin and reap. Behavioral, no prod-code test seam: the ordering (hardening before read) is observable because the child cannot pass the read while stdin stays open. The fail-closed branch is a compiler-checked `Result` propagation covered by review, not a test — forcing `setrlimit` failure would need exactly the kind of injection seam the codebase avoids.
   - `set_human_sops_failure_never_echoes_the_value_and_leaves_target_unchanged` — swap the fixture's `sops` symlink to `tests/fixtures/fake-sops-fail`; distinctive stdin value; assert nonzero exit, stdout/stderr contain no value bytes, `sops_calls() == 1`, no new file for the create case, and for the rotate case the pre-existing ciphertext bytes are unchanged (the staged tempfile was never persisted).
   - `set_human_never_echoes_the_value_to_stdout_or_stderr` — success path; assert the value bytes appear in neither stream (repo convention: tests assert absence of secret bytes).
   ...
   - `set_human_blocks_on_the_directory_lock` — the test opens the fixture's `secrets.human.d` directory and takes `Flock::lock(_, LockExclusive)` itself, spawns set-human via `command()` with stdin written and closed, sleeps ~300ms, asserts `try_wait()` is `None` (still blocked on the lock), drops the lock, then `wait_with_output()` succeeds and the file exists. Generous margins; this is a `tests/` integration test so no miri annotation is needed.
...
**Process hardening, first statements of `main()` (strictly before preflight and before any `/json/version` or `/cdp` I/O):** the cookie value will transit this process's memory, and this is a dotfiles utility installed on every machine, so it must not rely on inherited shell, systemd, or kernel core-dump configuration. Set `resource.setrlimit(resource.RLIMIT_CORE, (0, 0))` and `PR_SET_DUMPABLE=0` via `ctypes.CDLL(None, use_errno=True).prctl(4, 0, 0, 0, 0)`, checking both outcomes; if either fails, exit 1 with `browser-capture: could not disable core dumps` — fail closed, no browser access, no fallback. (`RLIMIT_CORE=0` is also inherited by the `secrets` child as a belt to Task 20's own braces.) Also pin library logging before any connection: `logging.getLogger("websockets").setLevel(logging.CRITICAL)` plus `propagate = False` and a `NullHandler`, so protocol/frame payloads can never reach stderr via Python's last-resort handler.

**Error model (mandatory — the leak surface is the failure path, not the happy path):**
- `class CaptureError(Exception)` carrying `exit_code: int` and `message: str`. Every expected failure below raises it. `main()` is a thin wrapper: `try: return run(args)`, `except CaptureError as e: print(e.message, file=sys.stderr); return e.exit_code`.
- A module-level flag `cdp_active` is set to True immediately before the first CDP command is sent. The top-level `except Exception` prints — when `cdp_active` — the fixed string `browser-capture: internal error (details withheld: a CDP response may hold cookie values)` and returns 1; before `cdp_active`, it prints `browser-capture: internal error: {type(e).__name__}` (class name only — never `str(e)`, never `repr`, never a traceback to the terminal).
- **No code anywhere formats a CDP response, target object, or cookie dict.** At each parse point, extract only the needed scalars into locals (`url`, `title`, `targetId`, `sessionId`, cookie `name`/`domain`/`path`/`expires`, and — only at step 5 — the value) and build every error listing exclusively from those scalars. The cookie value is bound to exactly one variable and appears in exactly one expression: the `input=` argument of the `secrets` subprocess call.
- **No `check=True`, no `CalledProcessError`** (its repr carries captured output), no `RuntimeError(f"... {response}")`, no logging of frames or responses; inspect `returncode` explicitly.

Flow (all errors to stderr; the only stdout ever is the final status line):
1. Preflight `secrets` availability before touching the browser: `subprocess.run(["secrets", "--help"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False)` and inspect the captured bytes. Classify as an old binary **only when the output looks like the secrets usage text** (contains `usage: secrets get KEY`) **and** lacks the substring `set-human` → exit 7: "secrets set-human is unavailable — the installed secretsd predates it; run: mise upgrade github:sjawhar/secretsd && bash ~/.dotfiles/installers/secretsd.sh". Output that does not look like usage (broken config, missing roots) → exit 7 "could not inspect the secrets CLI" plus the first line of that output — this avoids misdiagnosing config breakage as a missing release. A missing `secrets` binary (FileNotFoundError) → the same exit 7, loudly.
...
6. `subprocess.run(["secrets", "set-human", KEY, "--source", SOURCE], input=value.encode(), capture_output=True, check=False)`; nonzero `returncode` → exit 7, relaying the child's **stderr** verbatim (never its stdin, never `CalledProcessError`). Relaying stderr is deliberate rather than `DEVNULL`: the human debugging exit 7 needs the actual cause (conflicting `--source`, broken config, sops failure), and secretsd's stderr is guaranteed value-free by its own tested convention — `src/client/error.rs` never renders secret bytes and its tests assert their absence on stderr.
...
**The value never leaves the process**: not printed, not logged, no temp file, no `--print` flag, never in argv or env — stdin to `secrets set-human` is the only egress; core dumps are disabled before browser access; and with the error model above, no failure path can format an object that contains it.
...
d=$(mktemp -d) && printf '#!/bin/sh\necho "secrets: usage: secrets get KEY ..." >&2\nexit 1\n' > "$d/secrets" \
  && chmod +x "$d/secrets" && PATH="$d:$PATH" ~/.dotfiles/scripts/browser-capture \
     --domain x.test --cookie a --secret K 2>&1; echo "exit=$?"                 # exit 7, names mise upgrade (old-binary classification)
printf '#!/bin/sh\necho "secretsd config: no roots configured" >&2\nexit 1\n' > "$d/secrets" \
  && PATH="$d:$PATH" ~/.dotfiles/scripts/browser-capture \
     --domain x.test --cookie a --secret K 2>&1; echo "exit=$?"                 # exit 7, "could not inspect the secrets CLI" (not the old-binary message)
# Hardening applied before any network I/O — stall the /json/version GET and inspect the live process:
python3 -c 'import socket,time; s=socket.socket(); s.bind(("127.0.0.1",18123)); s.listen(); c,_=s.accept(); time.sleep(20)' & srv=$!
~/.dotfiles/scripts/browser-capture --domain x.test --cookie a --secret K --relay-url http://127.0.0.1:18123 & cap=$!
sleep 2; pid=$(pgrep -n -f 'browser-capture'); grep 'Max core file size' "/proc/$pid/limits"   # soft 0, hard 0
stat -c %U "/proc/$pid"                                                                        # root — dumpable cleared
kill $srv $cap 2>/dev/null; wait 2>/dev/null
# Leak canary — failure output must never contain a cookie value. Write a throwaway fake relay
# (NOT committed; the spec's real-tab e2e remains Task 29 — this is the negative security check):
#   /tmp/fake-cdp-relay.py: python websockets server on 127.0.0.1:18124 whose process_request answers
#   GET /json/version with 200 JSON, and whose /cdp handler replies: Target.getTargets -> one page
#   target {url: "https://x.test/", title: "Fake"}; Target.attachToTarget -> {sessionId: "S1"};
#   Network.getCookies -> {cookies: [{name: "other", value: "LEAKCANARY-9f3", domain: ".x.test",
#   path: "/", expires: -1}]}. Run it with: uv run --with 'websockets>=13' /tmp/fake-cdp-relay.py &
out=$(~/.dotfiles/scripts/browser-capture --domain x.test --cookie session --secret K \
      --relay-url http://127.0.0.1:18124 2>&1); echo "exit=$? (expect 5)"
printf '%s' "$out" | grep -c 'other'            # 1 — names are listed
printf '%s' "$out" | grep -c 'LEAKCANARY-9f3'   # 0 — the value never appears in failure output
# Repeat with the fake's getCookies handler returning malformed JSON: expect exit 1 and the exact
# line "browser-capture: internal error (details withheld: a CDP response may hold cookie values)",
# no traceback, and grep -c 'LEAKCANARY-9f3' -> 0.
```
**Laptop-required:** exit paths 3–4 against real grouped tabs and the success path — deliberately not mocked for the *positive* contract (the spec verifies this script end to end against a real grouped tab). That is Task 29. The commands above are the devbox proxy plus the negative leak checks.
...
- Daemon branch, after activation — materialize the Chrome extension, then **immediately override the tool's own next-steps output**, which currently ends with `Enable the mode:  omp config set browser.relay true` and would, if followed, flip the SHARED omp config default-on (`installers/omp.sh` symlinks the one committed `omp/config.yml` to `~/.omp/agent/config.yml` on every machine) — contradicting the locked per-call opt-in decision:
  ```bash
  ...
  ```
  `omp browser-relay install` writes the extension to `~/.omp/browser-relay/extension` (its `--dir` default, confirmed from `omp browser-relay --help`); re-running is idempotent. If Phase 2's fork work separates "install extension" from "enable default relay" in that printed output, the two override lines can shrink to the Chrome step alone — keep them until the installed omp release actually prints the amended text. Placement rationale: `laptop/install.sh` already delegates this whole role via `bash "${DOTFILES_DIR}/installers/forward.sh" daemon`, so the laptop needs no separate edit and the browser-relay deployment has one entry point.
...
HOME="$tmp" PATH="$tmp/bin:$PATH" DOTFILES_DIR="/home/ubuntu/.dotfiles" bash /home/ubuntu/.dotfiles/installers/forward.sh daemon 2>&1 | grep -c 'browser.relay intentionally stays false'   # 1 — the override line prints
...
- **Commit A (ungated):** `scripts/browser-capture`, `forward/omp-browser-relay`, `forward/omp-browser-relay.service`, `installers/forward.sh`, `omp/config.yml`. Describe: what the capture utility does (cookie → `secrets set-human`, value never printed, core dumps disabled), that the daemon role now installs two units plus the Chrome extension with the browser.relay-stays-false override note, and the relayUrl per-machine note from Task 27 (shared file, inert off-devbox, fail-closed by the peer check). `jj describe -m ...` then `jj new`.
...
3. **Devbox, the capture itself:**
   ...
   ```
   Expected stdout: exactly one line, `stored TEST_CAPTURE_COOKIE (domain ..., expires ...)`; expected file: `~/.dotfiles/.secrets/secrets.human.d/TEST_CAPTURE_COOKIE.local.env` (sops ciphertext, mode 0600). Confirm the store decrypts to the browser's value **without ever displaying it**, by comparing hashes: devbox `secrets TEST_CAPTURE_COOKIE -- sh -c 'printf %s "$TEST_CAPTURE_COOKIE" | sha256sum'` (this first read costs one YubiKey touch — the deliberate store-unattended/read-gated asymmetry the rollout step names) vs. the human computing the same sha256 over the cookie value shown in the laptop's devtools. Hashes match ⇒ capture stored the exact value; stdout carried only the status line.
