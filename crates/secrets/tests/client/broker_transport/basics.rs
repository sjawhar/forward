...
use secrets::client::{BrokerClient, BrokerResponse, ClientError, HumanClient};
use secrets::secret::SecretName;
...
fn human_get_sends_the_callers_tty_once_without_a_software_gate() {
    ...
    let result = client.get(&key, None);
...
fn get_with_no_request_lists_grants_without_sending_get() {
    ...
    assert_eq!(
        output.stdout,
        b"{\"key\":\"HUMAN\",\"tier\":\"human\",\"grant\":false,\"ttl\":null}\n"
    );
    assert_eq!(fixture.sops_calls(), 0);
    assert_eq!(broker.frames(), [hello(), "GRANTS".to_owned()]);
}
