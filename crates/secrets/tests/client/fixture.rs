...
"#,
        ...
        S: AsRef<std::ffi::OsStr>,
    {
        ...
        // A broken pipe here is not a test failure: several of these cases
        // assert that the client refuses *before* reading stdin, so the child
        // can legitimately exit first and the write loses the race. Under a
        // loaded machine it loses it often. Any other error is real.
        if let Err(error) = input.write_all(stdin) {
            assert_eq!(
                error.kind(),
                std::io::ErrorKind::BrokenPipe,
                "writing stdin to the client failed"
            );
        }
        drop(input);
