...
use super::{ErrCode, MAX_FRAME_BYTES, SUBSCRIBE_VERB};
...
pub enum Request {
    ...
    Get {
        ...
        tty: Option<String>,
        /// Requested lifetime for a freshly created grant, in seconds. `None`
        /// keeps today's behavior: the daemon's own default backstop.
        ttl_secs: Option<u64>,
    },
...
pub fn parse_request(line: &[u8]) -> Result<Request, ErrCode> {
    ...
    // `0` is never meaningful (parse_ttl on the client rejects it too), so a
    // present-but-zero or non-numeric `ttl` is malformed, not "no preference".
    let ttl_secs = |name: &str| -> Result<Option<u64>, ErrCode> {
        let Some(raw) = field(&fields, name) else {
            return Ok(None);
        };
        match raw.parse::<u64>() {
            Ok(0) | Err(_) => Err(ErrCode::BadRequest),
