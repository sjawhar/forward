...
use crate::{ErrCode, MAX_FRAME_BYTES};

/// A successfully decoded broker response.
#[derive(Clone, PartialEq, Eq)]
// Exhaustive on purpose: these enumerate the wire protocol, which is
// versioned. A new variant is a protocol change that every caller must
// handle deliberately -- a wildcard arm here would let one ship silently.
...
/// A timeout expressed as an error, so a slow peer is not a malformed one.
fn timed_out() -> ClientError {
    ClientError::Io(std::io::Error::from(std::io::ErrorKind::TimedOut))
}

/// Read one bounded line, never spending past `deadline` in total.
///
/// `read_until` cannot be used here: it loops internally, so a deadline
/// checked around the call is only observed once the peer decides to finish.
fn read_header_line(
    ...
    while line.len() <= MAX_FRAME_BYTES {
        ...
        match reader.read(&mut byte) {
            ...
            // A signal interrupted the read; the deadline check above bounds the retry.
...
/// Read a response, refusing to spend longer than `deadline` in total.
///
/// The socket's read timeout bounds a *single* read. It does not bound the
/// exchange: a peer sending one byte just inside each timeout refreshes it
/// indefinitely, so a caller in an accept loop would hold the connection open
/// for as long as the peer chooses.
pub(crate) fn read_response_by(
    stream: UnixStream,
    deadline: Option<std::time::Instant>,
) -> Result<BrokerResponse, ClientError> {
    let mut reader = BufReader::new(stream);
    let raw_header = read_header_line(&mut reader, deadline)?;
